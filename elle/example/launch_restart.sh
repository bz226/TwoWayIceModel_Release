...
START_FILE=results/fine_foam_step200
TOTALSTEPS=299       # Number of steps to perform in this simulation
                    # the total steps of all simulations = TOTALSTEPS +STARTSTEP
STARTSTEP=201         # Steps already performed in starting elle file
...
if [ $STARTSTEP -gt 0 ];
then
    $DEBUG cp $START_FILE.elle tmp.elle
    $DEBUG echo "emma copied start file"
fi
